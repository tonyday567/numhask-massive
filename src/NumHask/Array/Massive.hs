...
-- import Data.Massiv.Array.Manifest (Prim)
...
instance
  ...
  tabulate f =
    Array (A.makeVectorR A.B (V.ParN 0) (V.Sz1 (size s)) (f . shapen s))
    where
      s = shapeVal $ toShape @s
  {-# INLINE tabulate #-}

  index (Array m) i = m `V.index'` (flatten s i)
    where
      s = shapeVal (toShape @s)
  {-# INLINE index #-}
...
with ::
  ...
  (Array s a -> r) ->
  r
with (D.Array _ v) f = f (Array (V.fromList (A.ParN 0) $ Vector.toList v))
...
row i (Array a) = Array $ A.drop (A.Sz $ i * n) $ A.take (A.Sz n) a
  where
...
col i (Array a) = Array $ A.makeVectorR A.B (A.ParN 0) (A.Sz n) (\x -> a A.! (i + x * n))
  where
    n = fromIntegral $ natVal @n Proxy
